...
Dispatch the Agent tool with `model: "haiku"`. Do NOT pass `isolation: "worktree"` — the routing agent only reads a manifest and returns JSON; it never touches files or git, and worktree isolation fails outside a git repo. Use this prompt structure:
